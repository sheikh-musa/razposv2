...
const formatActivityData = (dataString: string) => {
  try {
    ...
    if (statusChange) {
      return (
        <div className="flex items-center gap-2">
          ...
          <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(statusChange[1] as string)}`}>{statusChange[1]}</span>
          <span className="text-gray-600">to</span>
          <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(statusChange[2] as string)}`}>{statusChange[2]}</span>
        </div>
      );
    }
    ...
    if (orderCompleteChange) {
      return <span className="text-gray-600">Updated Sales Order status to completed</span>;
    }
    ...
    if (paymentChange) {
      return <span className="text-gray-600">Updated payment reference</span>;
    }
...
const Tooltip = ({ content, targetRect }: { content: string; targetRect: DOMRect | null }) => {
  ...
  return createPortal(
    <div
      style={style}
      className="z-50 transform -translate-x-1/2 -translate-y-full -mt-2 px-3 py-2 text-xs bg-gray-900 text-white rounded-md shadow-lg whitespace-pre-wrap max-w-xs"
    >
      <div className="max-h-48 overflow-auto">{JSON.stringify(JSON.parse(content), null, 2)}</div>
          ...
          <div className="space-y-4">
            {activityLog.map((activity, index) => (
              <div key={index} className="flex items-start gap-3 py-3 hover:bg-gray-50 rounded-lg px-2">
                <SafeImage src={imageUrl} alt={activity.modified_by} width={32} height={32} className="rounded-full flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm text-black truncate">{activity.modified_by.split("@")[0]}</span>
                    <span className="text-gray-400 flex-shrink-0">•</span>
                    <span className="text-xs text-gray-500 flex-shrink-0">{formatDate(activity.creation)}</span>
                  </div>
                  <div className="mt-1">
                    <span
            ...
            {hoveredActivity && <Tooltip content={hoveredActivity.data} targetRect={hoveredActivity.rect} />}
          </div>
        </div>
      </div>
    </div>
